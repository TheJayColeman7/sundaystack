...
NFL fantasy football platform. Phase 0.5b done: two-team player trades (instant accept). Phase 0.5a ESPN-style FA/waivers and Phase 0.4 weekly H2H **points** are done. Extra commissioner tools and playoffs stay later in 0.5.
...
Apply `supabase/migrations` in order (foundation, fantasy leagues, drafts, weekly games, waivers, then trades). Against Neon from a network that blocks outbound 5432, use the HTTP/serverless driver (already selected when `DATABASE_URL` contains `neon.tech`).
...
pnpm test            # unit tests (normalizers, scoring presets, lineup, snake draft, fantasy points, waivers, trades)
pnpm typecheck
